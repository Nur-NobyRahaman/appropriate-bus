...
const Soney = ({ busName }) => {
    ...
    useEffect(() => {
        if (localStorage.getItem(`soneyTicket${time}`) != null) {
            const data = JSON.parse(localStorage.getItem(`soneyTicket${time}`))
            setSeatArray(data)
    ...
    useEffect(() => {
        if (clicked > 0) {
            localStorage.setItem(`soneyTicket${time}`, JSON.stringify(seatArray));
            handleNavigate()
        }
                ...
                <form action="" onSubmit={handleAddPayment} >
                    {
                        busName ?
                            <Box p={4} display={'flex'} flexDirection={'column'} justifyContent={'center'} alignItems={'center'}>
                                ...
                                <Button startIcon={<Done></Done>} fullWidth variant='contained' size='small' sx={{ mt: 1.5 }} color='success' disabled={origin === destination} onClick={() => handleClose('seat')}>Select Seat</Button>
                            </Box> : null
                    }
                    {
                        seatField ? <Box width={'20vw'} pb={2} mx={'auto'}  >
                            ...
                            <Box mt={2} >
                                ...
                                <Box p={4}>
                                    <Button startIcon={<DoneAll></DoneAll>} fullWidth sx={{ mt: 3, }} size='small' variant='contained' disabled={totalTrue?.length < 1} color='success' onClick={() => handleClose("select")}>Ticket selected</Button>
                                </Box>
